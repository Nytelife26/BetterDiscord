	...
	initConstructor () {
		...
		this.defaults = {
			settings: {
				...
				forceZeros:		{value:true, 	description:"Force leading zeros:"},
				otherOrder:		{value:false, 	description:"Show the time before the date (doesn't apply to custom format):"}
			},
			choices: {
				creationDateLang:		{value:"own", 	description:"Timestamp Format:"}
			},
			formats: {
				ownFormat:				{value:"$hour:$minute | $day/$month/$year", 	description:"Own Format:"}
			}
		};
	}

	getName () {return "Complete Timestamps";}

	getDescription () {return "Replace all timestamps with complete timestamps.";}

	getVersion () {return "1.2.9";}

	getAuthor () {return "DevilBro & Nytelife26";}
	...
	getTimestamp (languageid, time) {
		...
		if (languageid != "own") {
			...
			timestring = ownformat
				.split("$hour").join(settings.forceZeros && hour < 10 ? "0" + hour : hour)
				.split("$minute").join(minute < 10 ? "0" + minute : minute)
				.split("$second").join(second < 10 ? "0" + second : second)
				.split("$msecond").join(msecond)
				.split("$timemode").join(timemode)
				.split("$weekdayL").join(timeobj.toLocaleDateString(languageid,{weekday: "long"}))
				.split("$weekdayS").join(timeobj.toLocaleDateString(languageid,{weekday: "short"}))
				.split("$monthnameL").join(timeobj.toLocaleDateString(languageid,{month: "long"}))
				.split("$monthnameS").join(timeobj.toLocaleDateString(languageid,{month: "short"}))
				.split("$day").join(settings.forceZeros && day < 10 ? "0" + day : day)
				.split("$month").join(settings.forceZeros && month < 10 ? "0" + month : month)
				.split("$year").join(timeobj.getFullYear());
		}
		return timestring;
	}
